...
from PyQt5.QtGui import QIcon, QPainter, QColor, QPen, QCursor, QPixmap, QFont
...
class CodeInputWidget(QWidget, Ui_CodeInputWidget):
    ...
    class CodeButton(QPushButton):
        clicked_with_validity = pyqtSignal(bool)

        def __init__(self, text, validity):
            super().__init__(text)
        ...
        def _on_clicked(self):
            self.clicked_with_validity.emit(self.validity)
    ...
    def set_choices(self, choices, good_code):
        ...
        if not choices or not good_code:
            return
        height = round(math.sqrt(len(choices))) or 1
        for idx, choice in enumerate(choices):
            b = self.CodeButton(str(choice), choice == good_code)
            self.code_layout.addWidget(
                b, int(idx / height), idx % height, Qt.AlignHCenter | Qt.AlignVCenter
            )
            b.clicked_with_validity.connect(self._on_button_clicked)
