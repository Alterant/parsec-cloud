...
class Greeter:
    ...
    async def run(self, device, config, token):
        try:
            async with backend_authenticated_cmds_factory(
                ...
                await self.job_oob_send.send(in_progress_ctx.greeter_sas)
                ...
                try:
                    ...
                    await self.job_oob_send.send((True, None, in_progress_ctx.claimer_sas, choices))
                except Exception as exc:
                ...
                try:
                    human_handle, device_id, is_admin = await self.main_oob_recv.receive()
                    ...
                    print(str(exc), exc.__dict__, type(exc))
                    print(exc.status)
                    await self.job_oob_send.send((False, exc))
                except Exception as exc:
                    await self.job_oob_send.send((False, exc))
    ...
    async def get_greeter_sas(self):
        ...
        greeter_sas = await self.job_oob_recv.receive()
        return greeter_sas
    ...
    async def get_claimer_sas(self):
        ...
        r, exc, claimer_sas, choices = await self.job_oob_recv.receive()
        if not r:
    ...
    async def get_claim_requests(self):
        await self.main_oob_send.send(self.Step.GetClaimRequests)
        r, exc, human_handle, device_id = await self.job_oob_recv.receive()
        if not r:
            raise JobResultError(status="get-claim-request-failed", origin=exc)
        return human_handle, device_id

    async def create_new_user(self, human_handle, device_id, is_admin):
        await self.main_oob_send.send(self.Step.CreateNewUser)
        await self.main_oob_send.send((human_handle, device_id, is_admin))
        r, exc = await self.job_oob_recv.receive()
        if not r:
...
class GreetUserInstructionsWidget(QWidget, Ui_GreetUserInstructionsWidget):
    ...
    def _on_button_start_clicked(self):
        ...
        self.button_start.setText("TEXT_WAITING")
    ...
    def _on_wait_peer_error(self):
        ...
        if self.wait_peer_job.status != "cancelled":
            msg = "TEXT_GREET_USER_WAIT_PEER_ERROR"
            exc = None
...
class GreetUserCheckInfoWidget(QWidget, Ui_GreetUserCheckInfoWidget):
    ...
    def check_infos(self, _=""):
        if (
            ...
            and self.line_edit_user_email.text()
...
class GreetUserWidget(QWidget, Ui_GreetUserWidget):
    ...
    def exec_modal(cls, core, jobs_ctx, token, parent):
        ...
        d = GreyedDialog(w, _("TEXT_GREET_USER_TITLE"), parent=parent, width=800)
        w.dialog = d
